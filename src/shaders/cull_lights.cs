...
float test(vec3 minn, vec3 maxx, vec3 center, float r ){
    ...
    for(int i = 0; i < 3; ++i){
        float v = center[i];
        if(v < minn[i]){
            dist += (minn[i] - v) * (minn[i] - v);
        }
        if(v > maxx[i]){
            dist += (v - maxx[i]) * (v - maxx[i]);
        }
    }
...
void main()
{
    ...
    minn.y = min(
        min(-h1 + gl_GlobalInvocationID.y * h1 / 8, -h2 + gl_GlobalInvocationID.y * h2 / 8),
        min(-h1 + (gl_GlobalInvocationID.y + 1) * h1 / 8, -h2 + (gl_GlobalInvocationID.y + 1) * h2 / 8));
    maxx.y = max(
        max(-h1 + gl_GlobalInvocationID.y * h1 / 8, -h2 + gl_GlobalInvocationID.y * h2 / 8),
        max(-h1 + (gl_GlobalInvocationID.y + 1) * h1 / 8, -h2 + (gl_GlobalInvocationID.y + 1) * h2 / 8));
    float w1 = h1 * aspect;
    float w2 = h2 * aspect;
    minn.x = min(
        min(-w1 + gl_GlobalInvocationID.x * w1 / 8, -w2 + gl_GlobalInvocationID.x * w2 / 8),
        min(-w1 + (gl_GlobalInvocationID.x + 1) * w1 / 8, -w2 + (gl_GlobalInvocationID.x + 1) * w2 / 8));
    maxx.x = max(
        max(-w1 + gl_GlobalInvocationID.x * w1 / 8, -w2 + gl_GlobalInvocationID.x * w2 / 8),
        max(-w1 + (gl_GlobalInvocationID.x + 1) * w1 / 8, -w2 + (gl_GlobalInvocationID.x + 1) * w2 / 8));
    ...
    for(int i = 0; i < spot_cnt; i++){
        ...
        float intensity = light.position.w;
        float c = (5.0 * intensity - 1.0);
        float r = -1 + sqrt(0.14*0.14 + 4 * 0.07 * c) / 0.14;
        if(test(minn, maxx, light.position.xyz, r) > 0.5){
